...
void doubleSize(Vector *vector) {
  int *prevArr = vector->array;
  int *curArr = realloc(vector->array, sizeof(int) * 2 * vector->memorySize);
  if (curArr != NULL) {
    //free(prevArr);
    vector->array = curArr;
    vector->memorySize *= 2;
...
int insert(Vector *vector, int location, int value) {
  ...
  //if the array is full, double its mem size
  if (vector->size == vector->memorySize) {
    doubleSize(vector);
  }
  //shift index down by one
  int i;
  for (i = vector->size; i > location; i--) {
...
int delete(Vector *vector, int location) {
  ...
  //shift index up for each slot
  int i;
  for (i = location; i < vector->size - 1; i++) {
    vector->array[i] = vector->array[i + 1];
  }
  //free(&vector->array[i]);
  vector->size--;
